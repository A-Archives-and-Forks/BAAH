...
def get_now_running_app(use_config=None):
    ...
    if _is_PC_app(target_config.userconfigdict["SERVER_TYPE"]):
        ...
        if hwnd != 0:
            return config.userconfigdict["ACTIVITY_PATH"]
        else:
            return ""
...
def get_now_running_app_entrance_activity(use_config=None):
    ...
    target_config = config if not use_config else use_config
    if _is_PC_app(target_config.userconfigdict["SERVER_TYPE"]):
        return target_config.userconfigdict["ACTIVITY_PATH"]
...
def check_app_running(activity_path: str, printit = True) -> bool:
    """
    检查app是否在运行，不校验app的activity,只校验app的名字
    """
    # 靠 get_now_running_app 来识别有没有打开 PC ba
    try:
